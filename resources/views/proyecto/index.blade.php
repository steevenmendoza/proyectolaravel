<html>
  <head>
	...
	<body>
		...
		<h1 style="margin: 50px 100px">Usuarios Registrados </h1>
		<div class="container">
			<table class="table align-middle mb-0 bg-white" >
				...
				@foreach ($proyectos as $proyecto)
					<tr>
						...
						<td>
							<a href="{{ route('proyecto.edit', $proyecto->id) }}"><button type="button" class="btn btn-link btn-sm btn-rounded">Editar</button></a>
							<form action="{{ route('proyecto.destroy', $proyecto->id) }}" method="POST">
								@csrf
								@method('DELETE')
								<button type="submit" class="btn btn-link btn-sm btn-rounded" onclick="return confirm('¿Estás seguro de que deseas eliminar este proyecto?')">Eliminar</button>
							</form>
						</td>
					</tr>
				@endforeach
